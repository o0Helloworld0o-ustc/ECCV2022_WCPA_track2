...
if __name__ == '__main__':
    ...
    print('\nThe number of training images = %d' % train_dataset_size)
    ...
    # copy files
    filename = '%s_dataset.py' % train_opt.dataset_mode
    src_path = f'data/{filename}'
    dst_path = f'checkpoints/{train_opt.name}/{filename}'
    shutil.copyfile(src_path, dst_path)

    filename = '%s_model.py' % train_opt.model
    src_path = f'models/{filename}'
    dst_path = f'checkpoints/{train_opt.name}/{filename}'
    shutil.copyfile(src_path, dst_path)


    times = []
    for epoch in range(train_opt.epoch_count, train_opt.n_epochs + train_opt.n_epochs_decay + 1):
        ...
        for i, data in enumerate(train_dataset):  # inner loop within one epoch
            ...
            batch_size = data['img_local'].size(0)
